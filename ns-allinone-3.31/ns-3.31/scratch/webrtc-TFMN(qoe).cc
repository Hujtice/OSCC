...
struct FrameQoE {
    uint32_t frame_id;
    double bandwidth_utilization;  // U_F(i)
    double frame_delay;            // D_F(i) 帧发送延迟
    double frame_loss;             // loss_F(i) 帧丢包率
    double qoe;                    // QoE_i
    double mu_used;                // 该帧使用的mu值
    
    FrameQoE() : frame_id(0), bandwidth_utilization(0.0), 
                 frame_delay(0.0), frame_loss(0.0), qoe(0.0), mu_used(1.0) {}
    
    FrameQoE(uint32_t id, double bw_util, double delay, double loss, double q, double mu)
        : frame_id(id), bandwidth_utilization(bw_util), frame_delay(delay),
          frame_loss(loss), qoe(q), mu_used(mu) {}
};
...
public:
    RLStateManager() : current_mu(1.0), max_loss_rate(0.05), 
                      ...
                      hybrid_weight_(0.3), mu_min_(0.5), mu_max_(2.0),
                      adaptive_mu_enabled_(true) {
        ...
        std::cout << "  mu_min: " << mu_min_ << ", mu_max: " << mu_max_ << std::endl;
        std::cout << "  adaptive_mu_enabled: " << adaptive_mu_enabled_ << std::endl;
        std::cout << "==================================" << std::endl;
    ...
    void FinalizeCurrentRtGroup() {
        if (current_rt_group.packet_count > 0) {
            ...
            // ====== 新增：自适应mu值更新 ======
            // 当Rt分组完成时，根据规则更新mu值
            if (adaptive_mu_enabled_ && record.frame_id > 0) {
                UpdateMuBasedOnRtGroup(record);
            }
        }
    ...
    // ====== 新增：基于Rt分组的mu值更新 ======
    void UpdateMuBasedOnRtGroup(const RtGroupRewardRecord& current_record) {
        // 查找前一帧相同Rt的loss
        double prev_loss = 0.0;
        bool found_prev_loss = false;
        
        // 反向查找前一帧相同Rt的记录
        for (auto it = rt_group_records.rbegin(); it != rt_group_records.rend(); ++it) {
            if (it->frame_id == current_record.frame_id - 1 && it->Rt_value == current_record.Rt_value) {
                prev_loss = it->loss_rate;
                found_prev_loss = true;
                break;
            }
        }
        
        if (found_prev_loss) {
            // 规则1：比较loss(i,j)和loss(i-1,j)
            if (current_record.loss_rate > prev_loss) {
                current_mu -= 0.1;
                NS_LOG_INFO("Rt group mu update - loss increased: " << current_record.loss_rate 
                          << " > " << prev_loss << ", mu decreased to: " << current_mu);
            } else {
                current_mu += 0.1;
                NS_LOG_INFO("Rt group mu update - loss decreased: " << current_record.loss_rate 
                          << " <= " << prev_loss << ", mu increased to: " << current_mu);
            }
        } else {
            // 规则2：如果当前帧中出现Rt大于帧i-1中最大Rt
            // 查找前一帧的最大Rt
            uint32_t prev_max_rt = 0;
            for (const auto& record : rt_group_records) {
                if (record.frame_id == current_record.frame_id - 1) {
                    prev_max_rt = std::max(prev_max_rt, record.Rt_value);
                }
            }
            
            if (current_record.Rt_value > prev_max_rt) {
                current_mu += 0.1;
                NS_LOG_INFO("Rt group mu update - current Rt(" << current_record.Rt_value 
                          << ") > prev max Rt(" << prev_max_rt << "), mu increased to: " << current_mu);
            }
        }
        
        // 限制mu在[mu_min_, mu_max_]范围内
        current_mu = std::max(mu_min_, std::min(current_mu, mu_max_));
        NS_LOG_INFO("After Rt group update, mu clamped to: " << current_mu);
    }
    
    // ====== 新增：帧完成时的QoE计算和mu更新 ======
    void ProcessFrameCompletion(uint32_t frame_id, double bandwidth_utilization, 
                               double frame_delay, double frame_loss) {
        // 计算帧的QoE
        double frame_qoe = 0.2 * bandwidth_utilization - 0.2 * frame_delay - 0.3 * frame_loss;
        
        // 记录帧QoE
        FrameQoE qoe_record(frame_id, bandwidth_utilization, frame_delay, frame_loss, frame_qoe, current_mu);
        frame_qoe_records_[frame_id] = qoe_record;
        
        NS_LOG_INFO("Frame " << frame_id << " QoE calculated: U=" << bandwidth_utilization
                   << ", D=" << frame_delay << ", L=" << frame_loss << ", QoE=" << frame_qoe
                   << ", mu=" << current_mu);
        
        // 规则3：根据QoE_i和QoE_i-1比较更新mu
        if (adaptive_mu_enabled_ && frame_id > 0) {
            auto prev_it = frame_qoe_records_.find(frame_id - 1);
            if (prev_it != frame_qoe_records_.end()) {
                double prev_qoe = prev_it->second.qoe;
                if (frame_qoe > prev_qoe) {
                    current_mu += 0.1;
                    NS_LOG_INFO("Frame QoE mu update - current QoE(" << frame_qoe 
                              << ") > prev QoE(" << prev_qoe << "), mu increased to: " << current_mu);
                } else {
                    current_mu -= 0.1;
                    NS_LOG_INFO("Frame QoE mu update - current QoE(" << frame_qoe 
                              << ") <= prev QoE(" << prev_qoe << "), mu decreased to: " << current_mu);
                }
                
                // 限制mu在[mu_min_, mu_max_]范围内
                current_mu = std::max(mu_min_, std::min(current_mu, mu_max_));
                NS_LOG_INFO("After frame QoE update, mu clamped to: " << current_mu);
            }
        }
        
        // 输出到CSV文件
        OutputFrameQoEToCSV(qoe_record);
    }
    
    // ====== 新增：输出帧QoE到CSV文件 ======
    void OutputFrameQoEToCSV(const FrameQoE& qoe_record) {
        static bool header_written = false;
        std::ofstream file("Frame_QoE.csv", std::ios::app);
        
        if (!file.is_open()) {
            NS_LOG_ERROR("Cannot open Frame_QoE.csv for writing");
            return;
        }
        
        // 写入表头
        if (!header_written) {
            file << "frame_id,U_F(i),D_F(i),loss_F(i),QoE_i,mu_used" << std::endl;
            header_written = true;
        }
        
        // 写入数据
        file << qoe_record.frame_id << ","
             << qoe_record.bandwidth_utilization << ","
             << qoe_record.frame_delay << ","
             << qoe_record.frame_loss << ","
             << qoe_record.qoe << ","
             << qoe_record.mu_used << std::endl;
        
        file.close();
        
        NS_LOG_DEBUG("Frame QoE written to CSV: frame=" << qoe_record.frame_id
                    << ", QoE=" << qoe_record.qoe);
    }
    ...
    void PrintStatisticsSummary() const {
        ...
        std::cout << "Frame QoE records: " << frame_qoe_records_.size() << std::endl;
    ...
    // ====== 新增：获取帧QoE记录 ======
    const std::map<uint32_t, FrameQoE>& GetFrameQoERecords() const {
        return frame_qoe_records_;
    }
    
    // ====== 新增：设置mu边界 ======
    void SetMuBounds(double mu_min, double mu_max) {
        mu_min_ = std::max(0.1, mu_min);  // 确保最小值合理
        mu_max_ = std::max(mu_min_ + 0.1, mu_max);  // 确保最大值大于最小值
        NS_LOG_INFO("Mu bounds set: min=" << mu_min_ << ", max=" << mu_max_);
    }
    
    // ====== 新增：启用/禁用自适应mu ======
    void SetAdaptiveMuEnabled(bool enabled) {
        adaptive_mu_enabled_ = enabled;
        NS_LOG_INFO("Adaptive mu " << (enabled ? "enabled" : "disabled"));
    }
...
private:
    ...
    // ====== 新增：自适应mu相关 ======
    std::map<uint32_t, FrameQoE> frame_qoe_records_;  // 帧QoE记录
    double mu_min_;  // mu最小值
    double mu_max_;  // mu最大值
    bool adaptive_mu_enabled_;  // 是否启用自适应mu
};
...
public:
    ...
    void CompleteFrame(uint32_t frame_id) {
        ...
        // ====== 新增：计算帧的QoE并通知RL管理器 ======
        if (rl_manager_ != nullptr) {
            CalculateAndNotifyFrameQoE(frame_id);
        }
    ...
    // ====== 新增：计算帧的QoE并通知RL管理器 ======
    void CalculateAndNotifyFrameQoE(uint32_t frame_id) {
        auto it = frames.find(frame_id);
        if (it == frames.end()) {
            NS_LOG_ERROR("Frame " << frame_id << " not found for QoE calculation");
            return;
        }
        ...
        if (frame.total_packets == 0) {
            NS_LOG_WARN("Frame " << frame_id << " has no packets, skipping QoE calculation");
            return;
        }
        
        // 1. 计算带宽利用率 U_F(i)
        double bandwidth_utilization = CalculateFrameBandwidthUtilization(frame_id);
        
        // 2. 计算帧发送延迟 D_F(i)
        double frame_delay = CalculateFrameDelay(frame_id);
        
        // 3. 计算帧丢包率 loss_F(i)
        double frame_loss = CalculateFrameLossRate(frame_id);
        
        // 4. 通知RL管理器处理帧完成
        rl_manager_->ProcessFrameCompletion(frame_id, bandwidth_utilization, frame_delay, frame_loss);
        
        NS_LOG_INFO("Frame " << frame_id << " QoE parameters calculated: "
                   << "U=" << bandwidth_utilization << ", D=" << frame_delay 
                   << "s, L=" << frame_loss);
    }
    
    // ====== 新增：计算帧带宽利用率 ======
    double CalculateFrameBandwidthUtilization(uint32_t frame_id) {
        auto it = frames.find(frame_id);
        if (it == frames.end()) {
            return 0.0;
        }
        ...
        if (frame.first_packet_arrival_time == Seconds(0) || 
            frame.last_packet_arrival_time == Seconds(0)) {
            return 0.0;
        }
        
        // 计算帧发送持续时间
        Time frame_duration = frame.last_packet_arrival_time - frame.first_packet_arrival_time;
        if (frame_duration <= Seconds(0)) {
            return 0.0;
        }
        
        // 计算实际发送的数据量（字节）
        double actual_data_sent = frame.frame_size;  // 帧大小
        
        // 计算可用的带宽容量
        double available_capacity = 0.0;
        Time current_time = frame.first_packet_arrival_time;
        Time end_time = frame.last_packet_arrival_time;
        
        // 简单估计：使用帧期间的平均trace带宽
        if (m_bandwidth_changer) {
            // 获取帧开始和结束时的trace带宽
            uint32_t start_time_ms = static_cast<uint32_t>(frame.first_packet_arrival_time.GetMilliSeconds());
            uint32_t end_time_ms = static_cast<uint32_t>(frame.last_packet_arrival_time.GetMilliSeconds());
            
            TraceData start_data = m_bandwidth_changer->GetTraceDataAtTime(start_time_ms);
            TraceData end_data = m_bandwidth_changer->GetTraceDataAtTime(end_time_ms);
            
            double avg_trace_bw = (start_data.bandwidth + end_data.bandwidth) / 2.0;
            
            // 可用容量 = 平均带宽 * 持续时间
            available_capacity = avg_trace_bw * frame_duration.GetSeconds();
        } else {
            // 如果没有bandwidth_changer，使用默认值
            available_capacity = 20 * 1000000.0 * frame_duration.GetSeconds();  // 20 Mbps
        }
        
        // 带宽利用率 = 实际发送数据量 / 可用容量
        double utilization = (actual_data_sent * 8) / available_capacity;
        
        return std::min(std::max(utilization, 0.0), 1.0);
    }
    
    // ====== 新增：计算帧发送延迟 ======
    double CalculateFrameDelay(uint32_t frame_id) {
        auto it = frames.find(frame_id);
        if (it == frames.end()) {
            return 0.0;
        }
        ...
        // 帧发送延迟 = 最后一个包到达时间 - 第一个包到达时间
        Time frame_delay = frame.last_packet_arrival_time - frame.first_packet_arrival_time;
        
        return frame_delay.GetSeconds();
    }
    
    // ====== 新增：计算帧丢包率 ======
    double CalculateFrameLossRate(uint32_t frame_id) {
        auto it = frames.find(frame_id);
        if (it == frames.end()) {
            return 0.0;
        }
        ...
        // 帧丢包率 = 1 - (接收到的包数 / 总包数)
        double loss_rate = 1.0 - (static_cast<double>(frame.packets_received) / frame.total_packets);
        
        return std::min(std::max(loss_rate, 0.0), 1.0);
    }
                        ...
                        SenderLossStatistics* loss_statistics = nullptr)
{
    ...
    if (rl_manager != nullptr) {
        // 设置mu的边界值
        rl_manager->SetMuBounds(0.5, 2.0);
        // 启用自适应mu功能
        rl_manager->SetAdaptiveMuEnabled(true);
        NS_LOG_INFO("Adaptive mu enabled with bounds [0.5, 2.0]");
    }
                     ...
                     double loss_rate = 0.01, const std::string& video_trace_file = "")
{
    ...
    std::cout << "Adaptive mu feature: ENABLED with bounds [0.5, 2.0]" << std::endl;
    std::cout << "Frame QoE output: Frame_QoE.csv" << std::endl;
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (changer && !trace_filename.empty()) {
            ...
            std::cout << "  Adaptive mu: ENABLED" << std::endl;
            std::cout << "  Mu bounds: [0.5, 2.0]" << std::endl;
            std::cout << "==========================================" << std::endl;
        }
    ...
    for (int i=0;i<num;i++) {
        ...
        std::cout << "  - Adaptive mu adjustment with bounds [0.5, 2.0]" << std::endl;
        std::cout << "  - Frame QoE calculation and recording" << std::endl;
        std::cout << "  - Bandwidth scaling with μ=" << bandwidth_scale_factor << " (initial)" << std::endl;
    ...
    for (int i=0;i<num;i++) {
        ...
        // 输出帧QoE统计摘要
        const auto& frame_qoe_records = rl_managers[i]->GetFrameQoERecords();
        if (!frame_qoe_records.empty()) {
            std::cout << "Frame QoE records for session " << i+1 << ": " << frame_qoe_records.size() << " frames" << std::endl;
            
            // 计算平均QoE
            double total_qoe = 0.0;
            double max_qoe = -std::numeric_limits<double>::max();
            double min_qoe = std::numeric_limits<double>::max();
            
            for (const auto& record : frame_qoe_records) {
                total_qoe += record.second.qoe;
                max_qoe = std::max(max_qoe, record.second.qoe);
                min_qoe = std::min(min_qoe, record.second.qoe);
            }
            
            double avg_qoe = total_qoe / frame_qoe_records.size();
            std::cout << "  Average QoE: " << avg_qoe << std::endl;
            std::cout << "  Max QoE: " << max_qoe << std::endl;
            std::cout << "  Min QoE: " << min_qoe << std::endl;
            std::cout << "  Frame_QoE.csv file created with detailed records" << std::endl;
        }
    }
    ...
    
    {
        ...
        std::cout << "Bandwidth scale factor: μ=" << bandwidth_scale_factor << " (initial)" << std::endl;
    ...
    std::cout << "Adaptive mu experiment completed" << std::endl;
    std::cout << "Frame QoE records saved to Frame_QoE.csv" << std::endl;
    _exit(0);
}
                               ...
                               double bandwidth_scale_factor=1.0, const std::string& video_trace_file = "")
{
    ...
    std::cout << "Check Frame_QoE.csv for frame-level QoE metrics" << std::endl;
...
int main(int argc, char *argv[]){
    std::cout << "=== WebRTC TraceAll-Frame with Adaptive Mu and Frame QoE Analysis ===" << std::endl;
    ...
    std::cout << "Starting single trace simulation with adaptive mu feature..." << std::endl;
    std::cout << "Max bandwidth: " << mb << " Mbps" << std::endl;
    std::cout << "Loss rate: " << ls << std::endl;
    std::cout << "Initial bandwidth scale factor μ: " << mu << std::endl;
    std::cout << "Video trace file: " << (video_trace_file.empty() ? "not specified" : video_trace_file) << std::endl;
    std::cout << "Features:" << std::endl;
    std::cout << "  - Adaptive mu adjustment based on frame Rt-group loss comparison" << std::endl;
    std::cout << "  - Mu bounds: [0.5, 2.0]" << std::endl;
    std::cout << "  - Frame QoE calculation: QoE = 0.2*U - 0.2*D - 0.3*L" << std::endl;
    std::cout << "  - Frame QoE records saved to Frame_QoE.csv" << std::endl;
    std::cout << "  - Rt-group based reward calculation" << std::endl;
    ...
    std::cout << "=== WebRTC TraceAll-Frame with Adaptive Mu Completed Successfully ===" << std::endl;
    std::cout << "Frame QoE metrics saved to Frame_QoE.csv" << std::endl;
    _exit(0);
    return 0;
}