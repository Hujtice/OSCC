...
const uint32_t MAX_PACKETS_PER_FRAME = 150; 
...
public:
    ...
    struct FrameQoEDetail {
        ...
        FrameQoEDetail() : frame_id(0), bandwidth_utilization(0.0), loss_rate(0.0),
                          delay_metric(0.0), ddl_miss_rate(0.0), qoe_recv(0.0),
    ...
    void OnFrameComplete(uint32_t frame_id, double frame_qoe,
                      ...
                      double delay_metric, double ddl_miss_rate,
    ...
    // 获取帧QoE详细信息
    const std::map<uint32_t, FrameQoEDetail>& GetFrameQoEDetails() const {
        return frame_qoe_details_;
    }
    ...
    void OutputFrameQoE(const std::string& filename) const {
        ...
        file << "frame_id,bandwidth_utilization,loss_rate,delay_metric,ddl_miss_rate,"
        ...
        file.close();
        NS_LOG_INFO("OSCC frame QoE saved to: " << filename << " with " << frame_qoe_details_.size() << " frames");
    }
    ...
    // 强制输出所有帧QoE（即使没有完成）
    void ForceOutputAllFrameQoE(const std::map<uint32_t, FrameQoEDetail>& all_frame_details, const std::string& filename) const {
        std::ofstream file(filename);
        if (!file.is_open()) {
            NS_LOG_ERROR("Cannot open OSCC QoE file: " << filename);
            return;
        ...
        file << "frame_id,bandwidth_utilization,loss_rate,delay_metric,ddl_miss_rate,"
             << "qoe_recv,qoe_delay,qoe_loss,qoe_ddl,qoe,mu,timestamp_s" << std::endl;
        for (const auto& pair : all_frame_details) {
            const FrameQoEDetail& detail = pair.second;
            file << detail.frame_id << ","
                 << detail.bandwidth_utilization << ","
                 << detail.loss_rate << ","
        ...
        file.close();
        NS_LOG_INFO("OSCC all frames QoE saved to: " << filename << " with " << all_frame_details.size() << " frames");
    }
...
private:
    ...
    void AdjustMuIntraFrame(uint32_t frame_id, uint32_t Rt, double current_loss) {
        ...
        if (it != last_frame_rt_loss_.end()) {
            ...
            // 丢包率相同则不调整
        } else if (Rt > last_frame_max_rt_ && last_frame_max_rt_ > 0) {
            // 新出现的高Rt组（比上一帧最大Rt还大），探索性增加μ
            current_mu_ += epsilon_;
            adjusted = true;
...
public:
    ...
    struct FrameStatistics {
        ...
        std::vector<double> packet_delays;  // 新增：记录该帧每个包的延迟
    ...
    void ProcessPacketArrival(Time arrival_time, int64_t real_send_time_ms = -1) {
        ...
        // ==== 关键修改：计算并记录延迟到帧统计中 ====
        double delay_ms = (arrival_time - send_time).GetMilliSeconds();
        if (delay_ms < 0) delay_ms = 0;
        frame.packet_delays.push_back(delay_ms);
        // ==========================================
    ...
    void CompleteAllFrames() {
        ...
        std::cout << "=== Completing All Frames ===" << std::endl;
        std::cout << "Total frames in map: " << frames.size() << std::endl;
        ...
        for (auto& pair : frames) {
            ...
            if (!frame.frame_completed && frame.packets_received > 0) {
                ...
                // 计算QoE（即使帧没有完全接收）
                if (oscc_controller_) {
                    FrameQoEResult qoe_result = CalculateFrameQoE(frame.frame_id);
                    oscc_controller_->OnFrameComplete(
                        frame.frame_id, 
                        qoe_result.qoe,
                        qoe_result.bandwidth_utilization,
                    ...
                    if (rl_manager_) {
                        double new_mu = oscc_controller_->GetCurrentMu();
                        rl_manager_->SetMu(new_mu);
                        NS_LOG_DEBUG("OSCC: Updated RLStateManager mu to " << new_mu << " after frame " << frame.frame_id);
                    }
                }
                
                NS_LOG_INFO("Frame " << frame.frame_id << " force-completed: " 
                           << "deadline_miss=" << frame.missed_deadline 
                           << ", stall=" << frame.stall_duration.GetMilliSeconds() << "ms"
                           << ", delivery_ratio=" << (frame.packets_before_deadline * 100.0 / frame.total_packets) << "%"
                           << ", packets_received=" << frame.packets_received << "/" << frame.total_packets
                           << ", delay_data_points=" << frame.packet_delays.size());
                
                std::cout << "  Force-completed Frame " << frame.frame_id << ": " 
                          << frame.packets_received << "/" << frame.total_packets << " packets received, "
                          << "delay points: " << frame.packet_delays.size() << std::endl;
            }
        }
        std::cout << "=== Frame Completion Done ===" << std::endl;
    }
    ...
    FrameQoEResult CalculateFrameQoE(uint32_t frame_id) {
        ...
        // 3. QoE_loss: 丢包率（基于到达率）
        double loss_rate = 0.0;
        if (frame.total_packets > 0) {
            loss_rate = 1.0 - (static_cast<double>(frame.packets_received) / frame.total_packets);
        }
        ...
        // 保存QoE详细信息
        OSCCController::FrameQoEDetail detail;
        detail.frame_id = frame_id;
        detail.bandwidth_utilization = bandwidth_utilization;
        detail.loss_rate = loss_rate;
        detail.delay_metric = delay_metric;
        ...
        detail.qoe_ddl = qoe_ddl;
        detail.qoe = qoe;
        detail.mu = (oscc_controller_ ? oscc_controller_->GetCurrentMu() : 1.0);
        detail.timestamp = Simulator::Now();
        
        all_frame_qoe_details_[frame_id] = detail;
    ...
    // 获取所有帧QoE详细信息
    const std::map<uint32_t, OSCCController::FrameQoEDetail>& GetAllFrameQoEDetails() const {
        return all_frame_qoe_details_;
    }
...
private:
    ...
    OSCCController* oscc_controller_ = nullptr;
    std::map<uint32_t, double> frame_qoe_history_;  // 帧QoE历史
    std::map<uint32_t, OSCCController::FrameQoEDetail> all_frame_qoe_details_;  // 所有帧QoE详细信息
...
public:
    ...
    void OutputBandwidthStatistics(const std::string& filename, double loss_rate = 0.01) {
        ...
        // 修改表头，添加trace_bandwidth_bps、trace_bandwidth_mbps、trace_rtt_ms和trace_loss
