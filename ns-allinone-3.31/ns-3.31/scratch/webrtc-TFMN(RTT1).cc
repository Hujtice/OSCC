/*
 Full webrtc-TFMN(RTT).cc — integrated version (no omissions)
 - Uses FrameAwareWebrtcTrace, FrameManager, RLStateManager, BandwidthChanger
 - Removed incorrect 'override' annotations where base signatures may differ
 - Added trace -> sender SetTargetBitrate callback registration in InstallWebrtcApplication
 - Enhanced logging for OnBW and OnReceiptPktInfo
 - Intended to be compiled within your ns-3 + modified WebRTC environment
 NOTE: This file is long; please replace your existing webrtc-TFMN(RTT).cc with this content.
*/
...
public:
    ...
    // 处理数据包到达 - 修改为使用真实发送时间
    // 参数 real_send_time: 从 WebrtcTag 中获取的真实发送时间（毫秒）
    ...
    // 根据时间获取带宽记录（找到最接近的时间戳）    (continued...)
    ...
    void OutputBandwidthHistory(const std::string& filename) const {
        ...
        for (const auto& record : bandwidth_history_) {
            file << record.timestamp.GetSeconds() << ","
                 ...
                 << record.scaled_bandwidth << "," << record.mu_value << std::endl;
        }
...
public:
    ...
    // 新增：trace -> sender 回调（由 trace 通知 sender 下发 scaled 带宽）
    typedef Callback<void, uint32_t> SetTargetBitrateCallback;
    ...
    FrameAwareWebrtcTrace(FrameManager* frame_manager = nullptr, RLStateManager* rl_manager = nullptr, 
        ...
        : frame_manager_(frame_manager), rl_manager_(rl_manager), 
          ...
          m_changer(nullptr), m_lastRequestedBw(0), m_req_threshold_ratio(0.05), m_req_threshold_abs(1000) {
    ...
    // 新增：注册 trace -> sender 回调
    void SetSetTargetBitrateCallback(SetTargetBitrateCallback cb) {
        m_setTargetCb = cb;
        NS_LOG_INFO("FrameAwareWebrtcTrace: SetTargetBitrateCallback registered");
    }
    ...
    virtual void OnBW(uint32_t now, uint32_t bps) {
        ...
        if (rl_manager_) {
            ...
            std::cout << "[DEBUG] Got current mu from RL manager: " << current_mu << std::endl;
        } else {
        ...
        if (frame_manager_) {
            ...
            if (should_add) {
                ...
                std::cout << "  Scaled BW: " << scaled_bw << " bps (μ=" << current_mu << ")" << std::endl;
                std::cout << "  RTT from trace: " << trace_data.rtt << "ms" << std::endl;
                std::cout << "  Loss from trace: " << trace_data.loss << std::endl;
        ...
        // ===== 新增：调用 trace -> sender 回调（去抖判断） =====
        if (!m_setTargetCb.IsNull()) {
            bool should_invoke = true;
            if (m_lastRequestedBw > 0) {
                uint32_t old_bw = m_lastRequestedBw;
                uint32_t diff = (old_bw > scaled_bw) ? (old_bw - scaled_bw) : (scaled_bw - old_bw);
                if (diff < m_req_threshold_abs && diff < static_cast<uint32_t>(old_bw * m_req_threshold_ratio)) {
                    should_invoke = false;
                }
            }
            if (should_invoke) {
                m_setTargetCb(scaled_bw);
                m_lastRequestedBw = scaled_bw;
                std::cout << "[TRACE->SENDER] Request SetTargetBitrate: " << (scaled_bw/1000000.0)
                          << " Mbps (μ=" << current_mu << ") at " << now << "ms" << std::endl;
            } else {
                NS_LOG_DEBUG("FrameAwareWebrtcTrace::OnBW: scaled change too small, skip SetTargetBitrate");
            }
        }
        // ===== 新增结束 =====
    ...
    void OnScaledBandwidth(uint32_t now, uint32_t original_bps, uint32_t scaled_bps, double scale_factor) {
        ...
        // ===== 新增：在 OnScaledBandwidth 同样通知 sender（去抖判断） =====
        if (!m_setTargetCb.IsNull()) {
            bool should_invoke = true;
            if (m_lastRequestedBw > 0) {
                uint32_t old_bw = m_lastRequestedBw;
                uint32_t diff = (old_bw > scaled_bps) ? (old_bw - scaled_bps) : (scaled_bps - old_bw);
                if (diff < m_req_threshold_abs && diff < static_cast<uint32_t>(old_bw * m_req_threshold_ratio)) {
                    should_invoke = false;
                }
            }
            if (should_invoke) {
                m_setTargetCb(scaled_bps);
                m_lastRequestedBw = scaled_bps;
                std::cout << "[TRACE->SENDER] OnScaledBandwidth requests SetTargetBitrate: "
                          << (scaled_bps/1000000.0) << " Mbps (μ=" << scale_factor << ") at " << now << "ms" << std::endl;
            } else {
                NS_LOG_DEBUG("FrameAwareWebrtcTrace::OnScaledBandwidth: scaled change too small, skip SetTargetBitrate");
            }
        }
        // ===== 新增结束 =====
    }
...
private:
    ...
    // 新增：trace -> sender 回调及去抖变量
    SetTargetBitrateCallback m_setTargetCb;
    uint32_t m_lastRequestedBw;
    double m_req_threshold_ratio;
    uint32_t m_req_threshold_abs;
};
                        ...
                        VideoTraceManager* video_trace_manager = nullptr)
{
    ...
    // 注册 trace -> sender 的回调，使 trace 在 OnBW/OnScaledBandwidth 时请求 sender 下发 scaled bw
    if (trace && sendApp) {
        // trace 将在 OnBW 时调用 sendApp->SetTargetBitrate(scaled_bw)
        trace->SetSetTargetBitrateCallback(MakeCallback(&WebrtcSender::SetTargetBitrate, sendApp));
        std::cout << "[DEBUG] Registered trace -> sender callback for applying scaled bitrate" << std::endl;
    }
...
int main(int argc, char *argv[]){
    ...
    if (trace_file.empty()) {
        ...
        std::cerr << "Usage: ./waf --run \"scratch/webrtc-TFMN(All) --trace=<path> [--video_trace=<video_trace_path> --it=<instance> --folder=<output_dir> --mb=<bandwidth> --ls=<loss_rate> --mu=<scale_factor>]\"" << std::endl;
        return 1;
    }
