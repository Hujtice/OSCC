...
public:
    ...
    void OnPacketReceived(const FramePacketInfo& info, const FrameStatistics& frame_stats) {
        ...
        // 使用“系统真实丢包率”而不是 trace 中的 loss：
        // 如果启用了 OSCC，则使用 OSCCController 维护的滑动窗口丢包率，
        // 该窗口由 RTCP 反馈 / OnPacketLost / OnTransportFeedback 等真实事件更新，
        // 更接近 GCC 使用的实际 loss 观测。
        double effective_loss = trace_loss;
        if (oscc_controller_ && oscc_controller_->IsEnabled()) {
            effective_loss = oscc_controller_->GetCurrentWindowLoss();
        }
        rl_manager_->UpdateNetworkState(delay_ms, effective_loss, MilliSeconds(trace_rtt));
...
public:
    ...
    virtual void OnReceiptPktInfo(uint32_t now, uint32_t seq, uint32_t owd) {
        ...
        if (rl_manager_) {
            ...
            // 同 OnPacketReceived：优先使用 OSCC 的滑动窗口丢包率作为系统真实 loss_rate
            double effective_loss = trace_loss_rate;
            if (oscc_controller_ && oscc_controller_->IsEnabled()) {
                effective_loss = oscc_controller_->GetCurrentWindowLoss();
            }

            rl_manager_->UpdateNetworkState(current_delay_ms, effective_loss, MilliSeconds(trace_rtt_ms));
            NS_LOG_DEBUG("Updated current delay in RL manager: " << current_delay_ms << "ms, RTT=" 
                       << trace_rtt_ms << "ms, loss=" << effective_loss);
        }
                              ...
                              double current_loss_rate)
{
    ...
    // 更新网络状态 - 使用实际延迟和“系统丢包率”（优先来自 OSCC 滑动窗口）
    if (rl_manager != nullptr) {
        double effective_loss = trace_loss_rate;
        OSCCController* oscc = rl_manager->GetOSCCController();
        if (oscc && oscc->IsEnabled()) {
            effective_loss = oscc->GetCurrentWindowLoss();
        }
        rl_manager->UpdateNetworkState(current_delay, effective_loss, MilliSeconds(trace_rtt_ms));
    }
