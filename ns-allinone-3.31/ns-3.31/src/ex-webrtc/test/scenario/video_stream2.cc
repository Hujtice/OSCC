...
const int CallTest2::kDefaultWidth;
const int CallTest2::kDefaultHeight;
const int CallTest2::kDefaultFramerate;
const int CallTest2::kDefaultTimeoutMs = 30 * 1000;
const int CallTest2::kLongTimeoutMs = 120 * 1000;
const uint32_t CallTest2::kSendRtxSsrcs[kNumSsrcs] = {
    0xBADCAFD, 0xBADCAFE, 0xBADCAFF, 0xBADCB00, 0xBADCB01, 0xBADCB02};
const uint32_t CallTest2::kVideoSendSsrcs[kNumSsrcs] = {
    0xC0FFED, 0xC0FFEE, 0xC0FFEF, 0xC0FFF0, 0xC0FFF1, 0xC0FFF2};
const uint32_t CallTest2::kAudioSendSsrc = 0xDEADBEEF;
const uint32_t CallTest2::kFlexfecSendSsrc = 0xBADBEEF;
const uint32_t CallTest2::kReceiverLocalVideoSsrc = 0x123456;
const uint32_t CallTest2::kReceiverLocalAudioSsrc = 0x1234567;
const int CallTest2::kNackRtpHistoryMs = 1000;
namespace {
...
uint8_t CodecTypeToPayloadType(VideoCodecType codec_type) {
  switch (codec_type) {
    case VideoCodecType::kVideoCodecGeneric:
      return CallTest2::kFakeVideoSendPayloadType;
    case VideoCodecType::kVideoCodecVP8:
      return CallTest2::kPayloadTypeVP8;
    case VideoCodecType::kVideoCodecVP9:
      return CallTest2::kPayloadTypeVP9;
    case VideoCodecType::kVideoCodecH264:
      return CallTest2::kPayloadTypeH264;
    default:
      RTC_NOTREACHED();
  }
  return {};
...
VideoSendStream::Config CreateVideoSendStreamConfig(
  ...
  if (config.stream.use_rtx) {
    send_config.rtp.rtx.payload_type = CallTest2::kSendRtxPayloadType;
    send_config.rtp.rtx.ssrcs = rtx_ssrcs;
  }
  if (config.stream.use_flexfec) {
    send_config.rtp.flexfec.payload_type = CallTest2::kFlexfecPayloadType;
    send_config.rtp.flexfec.ssrc = CallTest2::kFlexfecSendSsrc;
    send_config.rtp.flexfec.protected_media_ssrcs = ssrcs;
  }
  if (config.stream.use_ulpfec) {
    send_config.rtp.ulpfec.red_payload_type = CallTest2::kRedPayloadType;
    send_config.rtp.ulpfec.ulpfec_payload_type = CallTest2::kUlpfecPayloadType;
    send_config.rtp.ulpfec.red_rtx_payload_type = CallTest2::kRtxRedPayloadType;
  }
  return send_config;
}
...
VideoReceiveStream::Config CreateVideoReceiveStreamConfig(
  ...
  if (config.stream.use_rtx) {
    ...
    recv.rtp.rtx_associated_payload_types[CallTest2::kSendRtxPayloadType] =
        CodecTypeToPayloadType(config.encoder.codec);
  }
  if (config.stream.use_ulpfec) {
    recv.rtp.red_payload_type = CallTest2::kRedPayloadType;
    recv.rtp.ulpfec_payload_type = CallTest2::kUlpfecPayloadType;
    recv.rtp.rtx_associated_payload_types[CallTest2::kRtxRedPayloadType] =
        CallTest2::kRedPayloadType;
  }
...
SendVideoStream2::SendVideoStream2(CallClient* sender,
                                 VideoStreamConfig config,
                                 Transport* send_transport,
...
SendVideoStream2::~SendVideoStream2() {
  sender_->SendTask(
      [this] { sender_->call_->DestroyVideoSendStream(send_stream_); });
}

void SendVideoStream2::Start() {
...
void SendVideoStream2::Stop() {
  sender_->SendTask([this] { send_stream_->Stop(); });
}

void SendVideoStream2::UpdateConfig(
...
void SendVideoStream2::UpdateActiveLayers(std::vector<bool> active_layers) {
...
bool SendVideoStream2::UsingSsrc(uint32_t ssrc) const {
...
bool SendVideoStream2::UsingRtxSsrc(uint32_t ssrc) const {
...
void SendVideoStream2::SetCaptureFramerate(int framerate) {
  sender_->SendTask([&] { video_capturer_->ChangeFramerate(framerate); });
}

VideoSendStream::Stats SendVideoStream2::GetStats() const {
  return send_stream_->GetStats();
}

ColumnPrinter SendVideoStream2::StatsPrinter() {
...
ReceiveVideoStream2::ReceiveVideoStream2(CallClient* receiver,
                                       VideoStreamConfig config,
                                       SendVideoStream2* send_stream,
  ...
  for (size_t i = 0; i < num_streams; ++i) {
    ...
    if (config.stream.use_flexfec) {
      ...
      flexfec.payload_type = CallTest2::kFlexfecPayloadType;
      flexfec.remote_ssrc = CallTest2::kFlexfecSendSsrc;
...
ReceiveVideoStream2::~ReceiveVideoStream2() {
...
void ReceiveVideoStream2::Start() {
  receiver_->SendTask([this] {
    for (auto* recv_stream : receive_streams_)
      recv_stream->Start();
...
VideoReceiveStream::Stats ReceiveVideoStream2::GetStats() const {
...
VideoStreamPair2::~VideoStreamPair2() = default;

VideoStreamPair2::VideoStreamPair2(CallClient* sender,
                                 CallClient* receiver,
                                 VideoStreamConfig config)
    : config_(config),
