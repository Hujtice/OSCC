...
// SendVideoStream2 provides an interface for changing parameters and retrieving
// states at run time.
class SendVideoStream2 {
 public:
  RTC_DISALLOW_COPY_AND_ASSIGN(SendVideoStream2);
  ~SendVideoStream2();
 ...
 private:
  ...
  SendVideoStream2(CallClient* sender,
...
// ReceiveVideoStream2 represents a video receiver. It can't be used directly.
class ReceiveVideoStream2 {
 public:
  RTC_DISALLOW_COPY_AND_ASSIGN(ReceiveVideoStream2);
  ~ReceiveVideoStream2();
  void Start();
  void Stop();
 ...
 private:
  friend class VideoStreamPair2;
  ReceiveVideoStream2(CallClient* receiver,
                     VideoStreamConfig config,
...
// VideoStreamPair2 represents a video streaming session. It can be used to
// access underlying send and receive classes. It can also be used in calls to
// the Scenario class.
class VideoStreamPair2 {
 public:
  RTC_DISALLOW_COPY_AND_ASSIGN(VideoStreamPair2);
  ~VideoStreamPair2();
